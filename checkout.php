...
<script type="text/javascript">
  $('#btn-finalizar').click(function(event) {
    ...
    if (frete == '40010') {
      $('#frete_selecionado').val('Sedex');
    }

    if (frete == '41106') {
      $('#frete_selecionado').val('PAC');
    }
    ...
    $.ajax({
      ...
      beforeSend: function() {
        ...
        $('#btn-finalizar').text('Carregando...');

        $('#mensagem-finalizar').text('Aguarde enquanto seu pedido é prcessado.');
      },
      success: function(msg) {
        console.log(msg);
        if (msg.trim() === 'Selecione um CEP válido para o Frete!') {
          ...
          // Redirecionar para a página com o ID da venda
      ...
      error: function() {
        ...
        $('#mensagem-finalizar').addClass('text-danger');
        $('#mensagem-finalizar').text('Ocorreu um erro ao processar a solicitação.');
      },
      complete: function() {
         // Desabilitar botão
...
// mostra ou apaga a opcao de calcular frete conforme o check
        $('#mensagem-finalizar').text("");
